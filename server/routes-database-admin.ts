  ...
  app.get("/api/admin/db/status", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const schemas = await pool.query(`
        SELECT
          schema_name,
          pg_size_pretty(SUM(pg_total_relation_size(quote_ident(schema_name) || '.' || quote_ident(table_name)))::bigint) as schema_size
        FROM information_schema.schemata s
        LEFT JOIN information_schema.tables t USING (schema_name)
